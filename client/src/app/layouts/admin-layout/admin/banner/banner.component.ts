  ...
  this.http.httpGet(constants.getallBanner).subscribe((response) => {
    ...
    if (response.code ==200) {
     ...
     this.bannerAllImages.forEach(element => {
       if(element.Type==1){
        this.tiles.push({text: 'One', cols: 1, rows: 1, color: '#f38003',url:element.ImgUrl,id:element.Id});
       }
       if(element.Type==5){
        this.gallery.push({text: 'One', cols: 1, rows: 1, color: '#f38003',url:element.ImgUrl,id:element.Id});
       }
      });
    }
